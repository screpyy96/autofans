...
-- Allow anyone to read objects in listing-images bucket (needed for visitors to view pictures)
drop policy if exists "select own objects" on storage.objects;
drop policy if exists "select listing images for all" on storage.objects;
create policy "select listing images for all"
