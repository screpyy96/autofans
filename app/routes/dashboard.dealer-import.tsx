...
export async function action({ request }: ActionFunctionArgs) {
  ...
  return redirectToImport(headers, {
    ...
    notice: `${importedCount} drafturi au fost create. Adaugă poze, apoi poți publica în masă.`,
  });
}
        ...
        <Card variant="elevated" padding="none" className="overflow-hidden">
          <div className="border-b border-white/10 bg-gradient-to-r from-accent-gold/15 to-transparent p-5 sm:p-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-300">Folosește template-ul ca să importăm corect prețurile, echiparea și localizarea.</p>
              </div>
            </div>
          </div>
          <div className="p-5 sm:p-6">
            ...
            <Form method="post" encType="multipart/form-data" className="rounded-2xl border border-dashed border-white/20 bg-white/[0.03] p-4 sm:p-5">
              ...
              <p className="mt-1 text-sm text-gray-400">Include toate coloanele din template. Coloanele lipsă sau rândurile invalide sunt raportate înainte de publicare.</p>
            ...
            <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.02] p-4 text-sm text-gray-300">
              ...
              <ol className="mt-2 list-decimal space-y-1 pl-5">
                ...
                <li>Adaugi fotografiile fiecărei mașini în editor.</li>
          ...
          {rows.map((row) => (
            <div key={row.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
              <div className="min-w-0">
                ...
                {row.errors.length > 0 ? <p className="mt-1 text-sm text-red-200">{row.errors.join(' ')}</p> : <p className="mt-1 text-sm text-gray-400">Draft creat — adaugă fotografii înainte de publicare.</p>}
              </div>
