...
export function meta() {
  return [{ title: 'Mesaje - AutoFans.ro' }];
}
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div className="grid min-h-[620px] overflow-hidden rounded-3xl border border-white/10 bg-secondary-900/80 shadow-2xl lg:grid-cols-[340px_1fr]">
        <aside className="border-b border-white/10 lg:border-b-0 lg:border-r">
          ...
          )) : <div className="p-8 text-center text-sm text-gray-400">Nu ai conversații încă. Deschide un anunț și apasă „Contactează vânzătorul”.</div>}
        </aside>
        <section className="flex min-h-[470px] flex-col">
          {activeConversation ? <>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto p-5">{messages.map((message: any) => <div key={message.id} className={`flex ${message.sender_id === userId ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${message.sender_id === userId ? 'bg-accent-gold text-secondary-950' : 'bg-white/10 text-white'}`}><p className="whitespace-pre-wrap">{message.body}</p><p className={`mt-1 text-[10px] ${message.sender_id === userId ? 'text-secondary-900/60' : 'text-gray-400'}`}>{new Date(message.created_at).toLocaleString('ro-RO', { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'short' })}</p></div></div>)}</div>
            <fetcher.Form method="post" className="border-t border-white/10 p-4"><input type="hidden" name="intent" value="send" /><input type="hidden" name="conversationId" value={activeConversation.id} /><div className="flex gap-3"><textarea name="body" maxLength={2000} required rows={2} placeholder="Scrie un mesaj…" className="min-h-12 flex-1 resize-none rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-sm text-white outline-none placeholder:text-gray-500 focus:border-accent-gold" /><Button type="submit" className="bg-gold-gradient text-secondary-900" aria-label="Trimite mesajul"><Send className="h-4 w-4" /></Button></div>{fetcher.data?.error && <p className="mt-2 text-sm text-red-300">{fetcher.data.error}</p>}</fetcher.Form>
