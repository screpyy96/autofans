...
`Secrets.xcconfig` este ignorat de Git. În CI, aceleași valori pot fi furnizate prin configurarea build settings `SUPABASE_URL` și `SUPABASE_ANON_KEY`. Operațiile mobile folosesc direct Supabase Edge Functions, nu URL-ul site-ului.
...
Catalog cu căutare, sortare, filtre și paginare; detaliu cu galerie și comparație; autentificare, creare cont, resetare parolă și Google OAuth; favorite, căutări salvate, alerte, mesaje, profil, recenzii, anunțuri și dashboard de vânzător. Mutațiile folosesc aceeași Supabase Edge Function `mobile-v1` ca Android.
